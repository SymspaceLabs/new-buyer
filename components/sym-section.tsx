...
const BlogCard = ({ title, subTitle, btnText, btnUrl }: BlogCardProps) => {
  return (
    <div
      className="rounded-[35px] sm:rounded-[50px] py-3 sm:py-5 px-3 sm:px-5 max-w-[500px] bg-white/30 shadow-[0_4px_30px_rgba(0,0,0,0.1)] backdrop-blur-[6.5px] border border-white relative overflow-hidden"
      style={{
        WebkitBackdropFilter: "blur(6.5px)",
      }}
    >
      <div className="relative z-[1] flex flex-col gap-[10px] sm:gap-[25px]">
        <h1 className="text-[16px] sm:text-[21px] text-[#2F2F2F] font-bold">
          {title}
        </h1>
        <p className="text-[12px] sm:text-base text-[#434167]">{subTitle}</p>
        <div className="flex justify-end">
          <Link href={btnUrl}>
            <Button
              className="rounded-[30px] text-[11px] text-[#2F2F2F] px-2 shadow-[0px_1px_24px_-1px_rgba(0,0,0,0.18)] backdrop-blur-[10px] bg-gradient-to-br from-white/50 via-[#e0e0e0]/29 to-[#c4c4c4]/10 hover:opacity-90"
              style={{
                background:
